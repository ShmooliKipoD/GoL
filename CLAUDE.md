...
**`GoL.Sim` must never reference MonoGame or MonoGame.Extended.** That is what
makes the sim unit-testable without a window and lets a 10 000-tick soak run
anywhere. Use `System.Numerics.Vector2` in the core; `GoL.Render` converts to
`Microsoft.Xna.Framework.Vector2` at the boundary.
`ArchitectureTests.SimAssembly_DoesNotReferenceGraphicsStack` enforces it — it
will fail exactly once, the day someone adds a convenient `using` for `Vector2`.

The same rule is why `MonoGame.Extended.ECS` and `Collisions.QuadTree` are
**deliberately unused** despite being tempting for the simulation. The sim gets
its own spatial hash.
